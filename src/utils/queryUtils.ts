...
export const filterBooks = (
  ...
  return books.filter((book) => {
    ...
    if (
      filters.publishedYearFrom &&
      book.publishedYear < filters.publishedYearFrom
    ) {
      return false;
    }

    if (
      filters.publishedYearTo &&
      book.publishedYear > filters.publishedYearTo
    ) {
      return false;
    }
...
export const getBookStats = (books: Book[], authors: Author[]) => {
  ...
  books.forEach((book) => {
    stats.booksByYear[book.publishedYear] =
      (stats.booksByYear[book.publishedYear] || 0) + 1;
  });
  ...
  stats.averagePublishedYear = Math.round(
